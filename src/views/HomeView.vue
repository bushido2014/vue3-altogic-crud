...
const updateBook = async (book, index) => {
  const { data, error } = await altogic.db
    .model('book')
    .object(book._id)
    .update({
      title: book.title,
...
<template>
  <main class="h-screen bg-slate-500">
    <div class="container max-w-screen-lg mx-auto py-8 px-4 flex flex-col">
      <h1 class="text-white text-4xl mb-6 font-bold">Altogic Crud App</h1>
      <ul class="grid gap-8 lg:grid-cols-3 md:grid-cols-2 grid-cols-1">
        <div
        ...
        >
          <div v-if="book.isEditing">
            ...
            <input
              type="text"
              v-model="book.title"
              class="mb-2 p-1 shadow-md block w-full"
            />
            <label>Author:</label>
            <input
              type="text"
              v-model="book.author"
              class="mb-4 p-1 shadow-md block w-full"
            />
          </div>
          <div v-else>
            <p class="text-xl mb-2 font-bold">{{ book.title }}</p>
            <p class="text-sm mb-6 font-bold">
              <span class="font-normal pr-2">By:</span>{{ book.author }}
            </p>
          </div>
